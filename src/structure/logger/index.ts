...
class Logger {
	...
	error(...message: (string | Error)[]) {
		let formattedMessages = message.map(m => m instanceof Error ? m.message : m);
		console.error(this.getFormat("ERROR"), this.createMessage(ColorText.ERROR, formattedMessages));
	}
